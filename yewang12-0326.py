#!/usr/bin/env python3
"""
Benchmark script to compare Flash Attention implementations:
- Triton (with deterministic=True)
- Triton (with deterministic=False)

Usage:
    python benchmark_varlen_mode_gqa_swa_meta_2.3B.py                       # Run timing benchmarks only
    python benchmark_varlen_mode_gqa_swa_meta_2.3B.py --config 0            # Run only config 0
    python benchmark_varlen_mode_gqa_swa_meta_2.3B.py --profile             # Profile kernels with rocprof + timing in one run
    python benchmark_varlen_mode_gqa_swa_meta_2.3B.py --profile --config 0  # Profile + benchmark only config 0
"""
...
#            b,  sq,    skv,   h,  h_k, d, causal, window_size
configs = [
            (4,  4096,  4096, 48,  6, 64, True,  (2047, 0)),
            (4,  4096,  8192, 48,  6, 64, True,  (2047, 0)),
            (4,  4096, 12288, 48,  6, 64, True,  (2047, 0)),
            (4,  4096, 16384, 48,  6, 64, True,  (2047, 0)),
            (4,  4096,  4096, 48,  6, 64, True,  (-1, -1)),
            (4,  4096,  8192, 48,  6, 64, True,  (-1, -1)),
            (4,  4096, 12288, 48,  6, 64, True,  (-1, -1)),
            (4,  4096, 16384, 48,  6, 64, True,  (-1, -1)),
]
...
def calculate_varlen_attention_tflops(batch_size, cu_seqlens_q, cu_seqlens_k, num_heads_q, head_dim_qk, fwd_time_ms, bwd_time_ms, is_causal, window_size):
    ...
    for batch_idx in range(batch_size):
      ...
      effective_sk = s_k if window_left < 0 else min(s_k, window_left + 1)
      ss_count = ss_count + s_q*effective_sk
...
def benchmark_config(batch_size, seqlen_q, seqlen_kv, nheads, nheads_k, headdim, causal, window_size, verbose_output=False):
    """Benchmark a single configuration."""
    results = {}

    q = torch.randn(batch_size, seqlen_q, nheads, headdim, device=device, dtype=dtype, requires_grad=True)
    k = torch.randn(batch_size, seqlen_kv, nheads_k, headdim, device=device, dtype=dtype, requires_grad=True)
    v = torch.randn(batch_size, seqlen_kv, nheads_k, headdim, device=device, dtype=dtype, requires_grad=True)

    query_padding_mask = generate_random_padding_mask(seqlen_q, batch_size, device, mode="random")
    key_padding_mask = generate_random_padding_mask(seqlen_kv, batch_size, device, mode="random")
    (
        q_unpad,
        k_unpad,
        v_unpad,
    ...
    if verbose_output:
        print(f"\n{'='*80}")
        print(f"Config: batch={batch_size}, seqlen_q={seqlen_q}, seqlen_kv={seqlen_kv}, nheads={nheads}, nheads_k={nheads_k}, "
              f"headdim={headdim}, causal={causal}, window_size={window_size}")
        print(f"{'='*80}")
    ...
    for i, (deterministic, key) in enumerate(det_modes):
        ...
        _, time_fwd = benchmark_forward(
            flash_attn_varlen_func, q_det, k_det, v_det, cu_seqlens_q_det, cu_seqlens_k_det, max_seqlen_q, max_seqlen_k,
        ...
        fwd_tflops, bwd_tflops = calculate_varlen_attention_tflops(
            batch_size, cu_seqlens_q_det.clone().detach(), cu_seqlens_k_det.clone().detach(), nheads, headdim, fwd_time_ms, bwd_time_ms, causal, window_size
        )
...
def print_summary_table(all_results):
    ...
    print("="*120)
    print(f"\n{'Config':<55} | {'Method':<20} | {'Fwd (ms)':<10} | {'Bwd (ms)':<10} | {'Fwd TF/s':<10} | {'Bwd TF/s':<10}")
    print("─"*120)

    for item in all_results:
        config = item['config']
        batch_size, seqlen_q, seqlen_kv, nheads, nheads_k, headdim, causal, window_size = config
        config_str = f"B={batch_size} Sq={seqlen_q} Skv={seqlen_kv} H={nheads} Hk={nheads_k} D={headdim} C={causal} W={window_size[0]}"

        results = item['results']

        r = results['nondet']
        print(f"{config_str:<55} | {'det=False':<20} | {r['fwd_time']*1e3:<10.3f} | {r['bwd_time']*1e3:<10.3f} | {r['fwd_tflops']:<10.2f} | {r['bwd_tflops']:<10.2f}")

        r = results['det']
        print(f"{config_str:<55} | {'det=True':<20} | {r['fwd_time']*1e3:<10.3f} | {r['bwd_time']*1e3:<10.3f} | {r['fwd_tflops']:<10.2f} | {r['bwd_tflops']:<10.2f}")
...
def save_results_csv(all_results):
    ...
    with open(csv_filename, 'w', newline='') as csvfile:
        ...
        writer.writerow([
            'TestID', 'Batch', 'SeqLen_Q', 'SeqLen_KV', 'NumHeads', 'NumHeads_k', 'HeadDim',
            'Causal', 'WindowSize', 'Deterministic',
        ...
        for item in all_results:
            ...
            for det_val, key in [(False, 'nondet'), (True, 'det')]:
                r = results[key]
                writer.writerow([
                    test_id, batch_size, seqlen_q, seqlen_kv,
                    nheads, nheads_k, headdim,
                    'TRUE' if causal else 'FALSE', window_size[0],
                    'TRUE' if det_val else 'FALSE',
...
def main(args):
    ...
    for idx in config_indices:
        ...
        batch_size, seqlen_q, seqlen_kv, nheads, nheads_k, headdim, causal, window_size = configs[idx]
        results = benchmark_config(batch_size, seqlen_q, seqlen_kv, nheads, nheads_k, headdim, causal, window_size, verbose_output=False)
        all_results.append({
            'config': (batch_size, seqlen_q, seqlen_kv, nheads, nheads_k, headdim, causal, window_size),
            'results': results
        })
