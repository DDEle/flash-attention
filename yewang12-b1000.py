...
configs = [
            (128, 4096, 16384,  16,   2, 64, True,  (2047, 0)),
            (128, 4096, 16384,  16,   2, 64, True,  (-1, -1)),
]

# Output directories
cwd = os.getcwd()
output_dir_name = "profiler_outputs"
output_csv = "benchmark_results_b1000.csv"
...
def benchmark_config(batch_size, seqlen_q, seqlen_kv, nheads, nheads_k, headdim, causal, window_size, verbose_output=False):
    ...
    for i, (deterministic, key) in enumerate(det_modes):
        ...
        _, bwd_tflops = calculate_varlen_attention_tflops(
            batch_size, cu_seqlens_q_det.clone().detach(), cu_seqlens_k_det.clone().detach(), nheads, headdim, bwd_time_ms, bwd_time_ms, causal, window_size
        )
...
def print_summary_table(all_results):
    ...
    print("="*120)
    print(f"\n{'Config':<55} | {'Method':<20} | {'Bwd (ms)':<10} | {'Bwd TF/s':<10}")
    print("─"*100)
    ...
    for item in all_results:
        ...
        r = results['nondet']
        print(f"{config_str:<55} | {'det=False':<20} | {r['bwd_time']*1e3:<10.3f} | {r['bwd_tflops']:<10.2f}")

        r = results['det']
        print(f"{config_str:<55} | {'det=True':<20} | {r['bwd_time']*1e3:<10.3f} | {r['bwd_tflops']:<10.2f}")
